...
// The TLS-interception root that some networks require clients to trust.
//
// This is the MEB 'fatihca' root (CN=fatihca, self-signed, valid to 2036) — a
// PUBLIC government CA, not a secret. It is baked in rather than read from disk
// because the server touches no filesystem and neither local/ nor *.cer is in
// the Docker image; reading it at runtime is the classic works-locally,
// MODULE_NOT_FOUND-on-Fly bug.
//
// Why it must live in the config's tlsSettings.certificates rather than the OS
// trust store: on Android, Go reads only /system/etc/security/cacerts, so a CA
// installed through Settings is invisible to Xray. And allowInsecure was
// removed in Xray v26.2.6, so trusting the CA is the only remaining remedy.
//
// Override with INTERCEPT_CA for a different network; empty means the generated
// config simply omits the certificates block.

const INTERCEPT_CA_PEM = [
  "-----BEGIN CERTIFICATE-----",
...
module.exports = { INTERCEPT_CA_PEM };
